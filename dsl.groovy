job("Groovy 1") 
	...
	scm {
		git {
			remote {
				github("skabhi003/devtask-6", "https")
			}
			branch("*/master")
		}
	}
...
job("Groovy 3") {
	...
	steps {
		shell("""status=\$(curl -sL -w "%{http_code}" -I "https://apache-skabhi003.cloud.okteto.net/" -o /dev/null)
if [[ \$status == 200 ]]
then
exit 0
else
...
description ("This is mailing job")
 authenticationToken('mail')
   publishers {
		mailer("skabhi005@gmail.com", false, false)
	}
   triggers {
